...
namespace QuickVR
{
    ...
    public abstract class QuickMirrorReflectionBase : MonoBehaviour
    {
        ...
        protected RenderTexture _reflectionTextureSinglePass = null;

        protected Material CopyTexMat;
        ...
        protected virtual void OnDisable()
        {
            ...
            DestroyImmediate(_reflectionTextureSinglePass);
            DestroyImmediate(CopyTexMat);
        ...
        protected virtual void CreateRenderTexture(ref RenderTexture result, string name, RenderTextureFormat format = RenderTextureFormat.Default, bool single=false)
        {
            if (!result || _reflectionQuality != _oldReflectionQuality)
            {
                ...
                result = new RenderTexture(size * (single ? 2 : 1), size, 16, format);
                result.name = name;
...
#region MIRROR RENDER
        ...
        public virtual void BeginCameraRendering(Camera cam)
        {
            ...
            else
            {
                ...
                ClearRenderTexture(_reflectionTextureSinglePass, Color.red);
            }
        }
        ...
        protected virtual void RenderReflection()
        {
            ...
            if (_updateMode == UpdateMode.Automatic || _requestRender)
            {
                ...
                if (UnityEngine.XR.XRSettings.enabled && UnityEngine.XR.XRSettings.eyeTextureDesc.vrUsage == VRTextureUsage.TwoEyes) // Single pass stereo rendering
                {
                    RenderVirtualImageStereo();

                    CopyTexMat.SetTexture("_LeftTex", _reflectionTextureLeft);
                    CopyTexMat.SetTexture("_RightTex", _reflectionTextureRight);
                    Graphics.Blit(null, _reflectionTextureSinglePass, CopyTexMat);

                    mat.SetTexture("_LeftEyeTexture", _reflectionTextureLeft);
                    mat.SetTexture("_RightEyeTexture", _reflectionTextureRight);
                    mat.SetTexture("_MainTex", _reflectionTextureSinglePass); // Both eye textures are packed into one in single-pass
                    mat.EnableKeyword("UNITY_SINGLE_PASS_STEREO");
                }
                else
                {
                    RenderVirtualImageStereo();
                    mat.SetTexture("_LeftEyeTexture", _reflectionTextureLeft);
                    mat.SetTexture("_RightEyeTexture", _reflectionTextureRight);
                    mat.DisableKeyword("UNITY_SINGLE_PASS_STEREO"); 
                }
